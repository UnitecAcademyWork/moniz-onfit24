...
import { LoadObjectivesController } from './load-all-objectives-controller'
import { LoadObjectives, ObjectiveModel } from './load-all-objectives-controller.protocol'
...
describe('LoadObjective Controller', () => {
  test('should call LoadObjectives', async () => {
    ...
    const loadSpy = jest.spyOn(loadObjectivesStub, 'loadAll')
    await sut.handle({})
  ...
  test('should return 204 returns empty', async () => {
    const { sut, loadObjectivesStub } = makeSut()
    jest.spyOn(loadObjectivesStub, 'loadAll').mockReturnValueOnce(Promise.resolve([]))
    const httpResponse = await sut.handle({})
  ...
  test('should return 500 if LoadObjectives', async () => {
    const { sut, loadObjectivesStub } = makeSut()
    jest.spyOn(loadObjectivesStub, 'loadAll').mockReturnValueOnce(Promise.reject(new Error()))
    const httpResponse = await sut.handle({})
